...
def create_dataset_for_completeness_baselines(args, model_type):
    args.json_file = os.path.join(args.base_path, "codebase", "Completeness_and_interventions", "paths_MoIE.json")
    save_path = os.path.join(
        args.output, args.dataset, "completeness", args.arch, "dataset", model_type,
    ...
    if args.dataset == "cub" or args.dataset == "awa2" or args.dataset == "HAM10k" or args.dataset == "SIIM-ISIC":
        ...
        print("=====>>> Test size:")
        print(test_tensor_features.size())
        print(test_tensor_y.size())
        print(test_mask_alpha.size())

        torch.save(test_tensor_features, os.path.join(save_path, f"test_tensor_features.pt"))
    ...
    if args.dataset == "cub":
        ...
        print("=====>>> Val size:")
        print(val_tensor_features.size())
        print(val_tensor_y.size())
        print(val_mask_alpha.size())
        torch.save(val_tensor_features, os.path.join(save_path, f"val_tensor_features.pt"))
        torch.save(val_tensor_y, os.path.join(save_path, f"val_tensor_y.pt"))
        torch.save(val_mask_alpha, os.path.join(save_path, f"val_mask_alpha.pt"))
    ...
    if args.dataset == "cub" or args.dataset == "HAM10k" or \
        ...
        print("=====>>> Train size:")
        print(train_tensor_features.size())
        print(train_tensor_y.size())
        print(train_mask_alpha.size())

        torch.save(train_tensor_features, os.path.join(save_path, f"train_tensor_features.pt"))
...
def main():
    ...
    for top_k in args.top_K:
        ...
        if args.model == "MoIE":
            ...
            create_dataset_for_completeness_baselines(args, model_type="Baseline_PostHoc")
