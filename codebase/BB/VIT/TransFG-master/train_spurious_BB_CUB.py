...
def valid(args, model, writer, test_loader, global_step):
    ...
    return val_accuracy, all_preds, all_label
...
def train(args, model, chk_pt_path, output_path, tb_logs_path):
    ...
    print(f"train_batch_size: {args.eval_batch_size}")
    print(args.train_batch_size)

    # Prepare dataset
    train_loader, test_loader = get_loader(args, waterbird_landbird=True)
        ...
        for step, batch in enumerate(epoch_iterator):
            ...
            if (step + 1) % args.gradient_accumulation_steps == 0:
                ...
                if global_step % args.eval_every == 0:
                    with torch.no_grad():
                        accuracy, all_eval_preds, all_eval_label = valid(args, model, writer, test_loader, global_step)
                    if args.local_rank in [-1, 0]:
                        if best_acc < accuracy:
                            print("saving model!!")
                            save_model(args, model, chk_pt_path, global_step)
                            best_acc = accuracy
                            np.save(os.path.join(args.output, "all_preds.npy"), all_eval_preds)
                            np.save(os.path.join(args.output, "all_label.npy"), all_eval_label)
                            print(f"outputs saved at {args.output}")
...
def main():
    ...
    parser.add_argument('--concept-names', nargs='+',
                        default=['has_bill_shape_dagger', 'has_bill_shape_hooked_seabird',
                                 ...
                                 'has_wing_pattern_multicolored', 'has_ocean', 'has_lake', 'has_bamboo', 'has_forest'])
