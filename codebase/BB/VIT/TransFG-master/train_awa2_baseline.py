...
def valid(args, criterion, model, writer, test_loader, global_step):
    # Validation!
    ...
    for step, batch in enumerate(epoch_iterator):
        ...
        with torch.no_grad():
            ...
            attribute = attribute.to(args.device, dtype=torch.float)
            logits = model(x)
            eval_loss = criterion(logits, attribute)
            eval_losses.update(eval_loss.item())
            preds = torch.sigmoid(logits)
            correct += get_correct_multi_label(preds, attribute)
...
def train(args, model, chk_pt_path, output_path, tb_logs_path, seed):
    ...
    while True:
        ...
        for step, batch in enumerate(epoch_iterator):
            ...
            attribute = attribute.to(args.device, dtype=torch.float)
            logits = model(x)
            loss = criterion(logits, attribute)
            preds = torch.sigmoid(logits)
            correct += get_correct_multi_label(preds, attribute)
            ...
            if (step + 1) % args.gradient_accumulation_steps == 0:
                ...
                if global_step % args.eval_every == 0:
                    with torch.no_grad():
                        accuracy = valid(args, criterion, model, writer, test_loader, global_step)
                    if args.local_rank in [-1, 0]:
