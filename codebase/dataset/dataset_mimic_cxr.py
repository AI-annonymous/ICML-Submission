...
class Dataset_mimic_for_explainer(Dataset):
    def __init__(
        ...
        # print(f"bb_logits shape: {self.bb_logits.size()}")
        if expert is not "baseline":
        ...
        elif  expert is "baseline":
            self.logits_concept_x = torch.load(os.path.join(dataset_path, f"{mode}_logits_concepts.pt"))
            self.proba_concept_x = torch.load(os.path.join(dataset_path, f"{mode}_proba_concepts.pt"))
            self.y = torch.load(os.path.join(dataset_path, f"{mode}_class_labels.pt"))
            self.y_one_hot = one_hot(self.y.to(torch.long)).to(torch.float)
            self.concepts = torch.load(os.path.join(dataset_path, f"{mode}_attributes.pt"))
        self.expert = expert
        self.mode = mode
