...
def cal_completeness_score_per_iter_baseline(args):
    random.seed(args.seed)
    np.random.seed(args.seed)
    torch.manual_seed(args.seed)
    start = time.time()
    train_loader, val_loader = get_data_loaders_per_iter_completeness_baseline(args)
    done = time.time()
    elapsed = done - start
    print("Time to the full datasets: " + str(elapsed) + " secs")
    g_chk_pt_path = os.path.join(
        args.checkpoints, args.dataset, "completeness", "baseline_posthoc", args.arch, "g_per_iter"
    )
    g_tb_logs_path = os.path.join(args.logs, args.dataset, "completeness", "baseline_posthoc", args.arch, "g_per_iter")
    g_output_path = os.path.join(args.output, args.dataset, "completeness", "baseline_posthoc", args.arch, "g_per_iter")
    os.makedirs(g_chk_pt_path, exist_ok=True)
    os.makedirs(g_tb_logs_path, exist_ok=True)
    os.makedirs(g_output_path, exist_ok=True)
    pickle.dump(args, open(os.path.join(g_output_path, "train_explainer_configs.pkl"), "wb"))
