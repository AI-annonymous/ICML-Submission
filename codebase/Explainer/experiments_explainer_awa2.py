...
def train_baseline_post_hoc(args):
    ...
    val_dataset = Dataset_awa2_for_explainer(
        dataset_path, "test_proba_concepts.pt", "test_class_labels.pt", "test_attributes.pt", "test_image_names.pkl",
        val_transform
    )
    ...
    logger = Logger_CUBS(
        1, g_chk_pt_path, g_tb_logs_path, g_output_path, train_loader, test_loader, len(args.labels), device
    )
    fit_baseline(
        args.epochs,
        model,
