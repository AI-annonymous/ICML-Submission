    ...
    for seed in range(args.seed):
        ...
        if args.dataset == "cub":
            ...
            train_dataset = Dataset_cub_for_explainer(
                dataset_path, "train_proba_concepts.pt", "train_class_labels.pt", "train_attributes.pt", train_data,
                train_transform
            )
        ...
        model, optimizer, g_chk_pt_path, g_tb_logs_path, g_output_path = setup_init_params(args, root, seed)
        cur_glt_chkpt = os.path.join(g_chk_pt_path, args.g_checkpoint)
        model.load_state_dict(torch.load(cur_glt_chkpt))
        model.eval()
        ...
        if test_loader is not None:
            predict_baseline(
                ...
                g_output_path,
                mode="test",
                logistic_explainer=args.logistic_explainer
            )
...
def predict_baseline(
    ...
    with torch.no_grad():
        with tqdm(total=len(val_loader)) as t:
            for batch_id, data_tuple in enumerate(val_loader):
                if dataset == "cub":
                    val_images, val_concepts, attributes, val_y, val_y_one_hot = data_tuple
                    ...
                    val_images, val_concepts, attributes, _, val_y, val_y_one_hot = data_tuple
                    val_images, val_concepts, val_y, val_y_one_hot = val_images.to(device), \
                                                                     val_concepts.to(device), \
                                                                     val_y.to(torch.long).to(device), \
                                                                     val_y_one_hot.to(device)
                    attributes = attributes.to(torch.float32).to(device)
                if logistic_explainer == "n":
                    y_hat, concept_mask, \
                    alpha, alpha_norm, \
                    conceptizator = model(val_concepts)
                else:
                    y_hat = model(val_concepts)
                    concept_mask = model.linear.weight
                    alpha = model.linear.weight
                    alpha_norm = model.linear.weight
                    conceptizator = model.linear.weight

                bb_logits, feature_x = get_phi_x(val_images, bb, args.arch, args.layer)
                t.set_postfix(epoch='{0}'.format(batch_id))
                t.update()

                # tensor_images = torch.cat((tensor_images, val_images.cpu()), dim=0)
                tensor_features = torch.cat((tensor_features, feature_x.cpu()), dim=0)
                tensor_concepts = torch.cat((tensor_concepts, val_concepts), dim=0)
                tensor_preds = torch.cat((tensor_preds, y_hat), dim=0)
                tensor_attributes = torch.cat((tensor_attributes, attributes), dim=0)

                tensor_y = torch.cat((tensor_y, val_y), dim=0)
                tensor_conceptizator_concepts = torch.cat(
                    (tensor_conceptizator_concepts, conceptizator.concepts), dim=1
                ) if logistic_explainer == "n" else None
    ...
    tensor_conceptizator_concepts = tensor_conceptizator_concepts.cpu() if logistic_explainer == "n" else None
...
def train_baseline_post_hoc(args):
    for seed in range(args.seed):
        ...
        logger = Logger_CUBS(
            1, g_chk_pt_path, g_tb_logs_path, g_output_path, train_loader, val_loader, len(args.labels),
            utils.get_device()
        )

        fit_baseline(
            args.logistic_explainer,
            args.dataset,
            args.epochs,
            model,
            optimizer,
    ...
    for epoch in range(epochs):
        ...
        with tqdm(total=len(train_loader)) as t:
            for batch_id, data_tuple in enumerate(train_loader):
                if dataset == "cub":
                    train_images, train_concepts, _, train_y, train_y_one_hot = data_tuple
                    ...
                    train_images, train_concepts, _, _, train_y, train_y_one_hot = data_tuple
                    train_images, train_concepts, train_y, train_y_one_hot = train_images.to(device), \
                                                                             train_concepts.to(device), \
                                                                             train_y.to(torch.long).to(device), \
                                                                             train_y_one_hot.to(device)

                y_hat, _, _, _, _ = model(train_concepts)
                if logistic_explainer == "n":
        ...
        with torch.no_grad():
            with tqdm(total=len(val_loader)) as t:
                for batch_id, data_tuple in enumerate(val_loader):
                    if dataset == "cub":
                        val_images, val_concepts, _, val_y, val_y_one_hot = data_tuple
                        val_images, val_concepts, val_y, val_y_one_hot = val_images.to(device), \
                                                                         val_concepts.to(device), \
                                                                         val_y.to(torch.long).to(device), \
                                                                         val_y_one_hot.to(device)
                    elif dataset == "awa2":
                        val_images, val_concepts, _, _, val_y, val_y_one_hot = data_tuple
                        val_images, val_concepts, val_y, val_y_one_hot = val_images.to(device), \
                                                                         val_concepts.to(device), \
                                                                         val_y.to(torch.long).to(device), \
                                                                         val_y_one_hot.to(device)
                    y_hat, _, _, _, _ = model(val_concepts)
