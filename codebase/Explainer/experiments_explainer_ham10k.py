...
def train_baseline_post_hoc(args):
    for seed in range(args.seed):
        ...
        root = f"lr_{args.lr[0]}_epochs_{args.epochs}_temperature-lens_{args.temperature_lens}" \
               ...
               f"_lambda-lens_{args.lambda_lens}" \
               f"_hidden-layers_{len(args.hidden_nodes)}"
        ...
        chk_pt_explainer = os.path.join(args.checkpoints, args.dataset, "Baseline_PostHoc", "seeds", f"seed_{seed}",
                                        root)
        g_chk_pt_path = os.path.join(args.checkpoints, args.dataset, "Baseline_PostHoc", root)
        g_output_path = os.path.join(args.output, args.dataset, "Baseline_PostHoc", root)
        g_tb_logs_path = os.path.join(args.logs, args.dataset, "Baseline_PostHoc", root)
        ...
        logger = Logger_MIMIC_CXR(
            1, best_auroc, start_epoch, g_chk_pt_path, g_tb_logs_path, g_output_path, train_loader, val_loader,
        ...
        print(output_path)
        model.load_state_dict(torch.load(os.path.join(g_chk_pt_path, "best_model.pth.tar"))["state_dict"])
        model.eval()
        ...
        predict_baseline(
            bb_model_bottom, model, concept_bank, val_loader, mode="val", output_path=output_path, device=device
        )

        predict_baseline(
            bb_model_bottom, model, concept_bank, train_loader, mode="train", output_path=output_path, device=device
        )
    ...
    for epoch in range(args.epochs):
        ...
        with tqdm(total=len(train_loader)) as t:
            for batch in enumerate(train_loader):
                ...
                train_loss = criterion(y_hat, target) + lambda_lens * entropy_loss_elens
                optimizer.zero_grad()
                train_loss.backward()
                optimizer.step()

                logger.track_train_loss(train_loss.item())
                logger.track_total_train_correct_per_epoch(y_hat, target)
                t.set_postfix(
        ...
        with torch.no_grad():
            with tqdm(total=len(val_loader)) as t:
                for batch in enumerate(val_loader):
                    ...
                    entropy_loss_elens = entropy_loss(model)
                    val_loss = criterion(y_hat, target) + lambda_lens * entropy_loss_elens

                    logger.track_val_loss(val_loss.item())
                    logger.track_total_val_correct_per_epoch(y_hat, target)
                    logger.track_val_bb_outputs(out_class=y_hat, val_y=target)
                    t.set_postfix(
        ...
        with tqdm(total=len(val_loader)) as t:
            for batch in enumerate(val_loader):
                ...
                with torch.no_grad():
                    ...
                    val_concepts = compute_dist(concept_bank, phi)
                y_hat = model(val_concepts)
                # tensor_images = torch.cat((tensor_images, images.cpu()), dim=0)
