...
def get_initial_setups(args, model_type):
    ...
    return train_loader, val_loader, g_chk_pt_path, g_output_path, g_tb_logs_path, results_path
...
def setup_pretrained_models_cv(args):
    ...
    op_size = len(args.landmark_names_spec) + len(args.abnorm_obs_concepts) if args.dataset == "mimic_cxr" else len(
        get_n_labels_concepts(args)["concepts"]
    )
...
def compute_completeness_score(args, g_completeness, classifier, cav, g_output_path, test_loader, device):
    g_completeness.eval()
    ...
    if args.dataset == "awa2" or args.dataset == "cub":
        ...
        completeness_score_auroc = (val_auroc_completeness - 0.5) / (val_auroc_bb - 0.5)
