...
sys.path.append(
    os.path.abspath("root-path/codebase")
)
...
def get_out_paths(args):
    ...
    elif args.model == "Baseline_CBM_logic":
        base_path = paths[f"{args.dataset}_{args.arch}"]["baseline_cbm_logic_paths"]["base_path"]
        files = paths[f"{args.dataset}_{args.arch}"]["files"]
        full_output_path = os.path.join(args.output, args.dataset, "Baseline", base_path, "explainer")
        return files, full_output_path
    elif args.model == "Baseline_PCBM_logic":
        base_path = paths[f"{args.dataset}_{args.arch}"]["baseline_pcbm_logic_paths"]["base_path"]
        files = paths[f"{args.dataset}_{args.arch}"]["files"]
        full_output_path = os.path.join(args.output, args.dataset, "Baseline_PostHoc", base_path)
        return files, full_output_path
...
def checkpoint_paths(args):
    ...
    if args.model == "MoIE":
        ...
        base_path = paths[f"{args.dataset}_{args.arch}"]["baseline_cbm_logic_paths"]["base_path"]
        checkpoint_file = paths[f"{args.dataset}_{args.arch}"]["baseline_cbm_logic_paths"]["checkpoint_g"]
        checkpoint = os.path.join(
            args.checkpoints, args.dataset, "Baseline", base_path, "explainer", checkpoint_file
        )
        test_config = os.path.join(
            args.output, args.dataset, "Baseline", base_path, "explainer", "test_explainer_configs.pkl",
        )
        return checkpoint, test_config
    elif args.model == "Baseline_PCBM_logic":
        base_path = paths[f"{args.dataset}_{args.arch}"]["baseline_pcbm_logic_paths"]["base_path"]
        checkpoint_file = paths[f"{args.dataset}_{args.arch}"]["baseline_pcbm_logic_paths"]["checkpoint_g"]
        checkpoint = os.path.join(
            args.checkpoints, args.dataset, "Baseline_PostHoc", base_path, checkpoint_file
        )
        test_config = os.path.join(
            args.output, args.dataset, "Baseline_PostHoc", base_path, "test_explainer_configs.pkl",
        )
        return checkpoint, test_config
...
def get_model(pkl, args, device, checkpoint):
    if (args.dataset == "HAM10k" or args.dataset == "SIIM-ISIC") and args.model == "Baseline_PCBM_logic":
        moIE = Explainer(
            n_concepts=len(pkl.concept_names), n_classes=len(pkl.labels), explainer_hidden=pkl.hidden_nodes,
            conceptizator=pkl.conceptizator, temperature=pkl.temperature_lens,
        ).to(device)
        moIE.load_state_dict(torch.load(checkpoint)["state_dict"])
    else:
...
def intervene_and_pred_alt_results(args, test_tensor_y, test_tensor_preds, tensor_alpha_norm, test_tensor_concepts):
    ...
    if args.model == "Baseline_CBM_logic":
        pkl.input_size_pi = 2048
    ...
    with torch.no_grad():
        for idx in range(test_tensor_y.size(0)):
            ...
            if (args.dataset == "HAM10k" or args.dataset == "SIIM-ISIC") and args.model == "Baseline_PCBM_logic":
                y_pred_ex = moIE(concepts.unsqueeze(0).to(device))
            else:
